<template>
    ...
    <div class="file-drop mt-3">
        <div id="label" class="label subheading empty">{{label}}</div>
        ...
        >
            <div>
                ...
                <input class="file" type="file" name="files[]" id="file" data-multiple-caption="{count} files selected" multiple v-on:change="onFilesSelected"/>
                <label for="file">
                    <span v-if="!errorMessage && droppedFiles.length === 0">
                        <strong>Choose a file</strong><span class="dragndrop"> or drag it here</span><br>
                        <span class="caption">(Authorized formats are JPEG, JPG, PNG, PDF, ZIP, WORD, XLS, PPT)</span>
                    </span>
    ...
    methods: {
        ...
        onFilesSelected(e) {
            ...
            if(!this.isValidType(files[0])) {
                ...
                this.file = files[0]
                document.querySelector('#label').classList.remove('empty')
            }
        }
    }
}
